...
public class AuthServiceTests
{
    ...
    public AuthServiceTests()
    {
        ...
        _sut = new AuthService(
            _db, _passwordHasherMock.Object, _tokenServiceMock.Object, _loggerMock.Object
        );
    }
    ...
    public async Task RegisterAsync_ShouldCreateUserAndReturnTokens()
    {
        ...
        _tokenServiceMock.Setup(t => t.GenerateAccessToken(It.IsAny<User>(), null))
    ...
    {
        _db.Users.Add(new User { Id = Guid.NewGuid(), Username = "existing", Email = "other@example.com", PasswordHash = "hash" });
    ...
    {
        _db.Users.Add(new User { Id = Guid.NewGuid(), Username = "other", Email = "test@example.com", PasswordHash = "hash" });
        await _db.SaveChangesAsync();
    ...
    public async Task LoginAsync_ShouldReturnTokens_WhenCredentialsAreValid()
    {
        ...
        _tokenServiceMock.Setup(t => t.GenerateAccessToken(It.IsAny<User>(), null)).Returns("access-token");
        _tokenServiceMock.Setup(t => t.GenerateRefreshToken()).Returns(("raw-refresh", "hashed-refresh"));
    ...
    public async Task LoginAsync_ShouldThrow_WhenPasswordIsWrong()
    {
        ...
        _passwordHasherMock.Setup(p => p.Verify("wrong-password", "hashed-password")).Returns(false);

        var request = new LoginRequestDto { Email = "test@example.com", Password = "wrong-password" };
    ...
    public async Task RefreshAsync_ShouldReturnNewTokens()
    {
        ...
        _tokenServiceMock.Setup(t => t.GenerateAccessToken(It.IsAny<User>(), null)).Returns("new-access-token");
        _tokenServiceMock.Setup(t => t.GenerateRefreshToken()).Returns(("new-raw-refresh", "new-hashed-refresh"));
    ...
    {
        var userId = Guid.NewGuid();
        _db.Users.Add(new User { Id = userId, Username = "u", Email = "u@example.com" });
        var storedToken = new RefreshToken
        {
            Id = Guid.NewGuid(),
            UserId = userId,
    ...
    {
        var userId = Guid.NewGuid();
        _db.Set<RefreshToken>().AddRange(
            new RefreshToken { Id = Guid.NewGuid(), UserId = userId, TokenHash = "hash1", RevokedAt = null, ExpiresAt = DateTime.UtcNow.AddDays(1) },
            new RefreshToken { Id = Guid.NewGuid(), UserId = userId, TokenHash = "hash2", RevokedAt = null, ExpiresAt = DateTime.UtcNow.AddDays(1) }
        );
        await _db.SaveChangesAsync();

        await _sut.LogoutAsync(userId);

        _db.Set<RefreshToken>().Where(rt => rt.UserId == userId).All(rt => rt.RevokedAt != null).Should().BeTrue();
    }
    ...
    public async Task GetCurrentUserAsync_ShouldIncludePartner_WhenCoupled()
    {
        ...
        _db.Users.AddRange(
            new User { Id = userId, Username = "user1", Email = "user1@example.com" },
            new User { Id = partnerId, Username = "user2", Email = "user2@example.com" }
        );
        _db.Couples.Add(new Couple { Id = coupleId, User1Id = userId, User2Id = partnerId });
        await _db.SaveChangesAsync();
